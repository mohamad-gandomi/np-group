...
export function ProductCard({ product, linkToDetail = true }: { product: Product; linkToDetail?: boolean }) {
  const content = (
    <>
      ...
      <div className="flex flex-1 flex-col border-x border-b border-black/[0.07] bg-white px-3 pb-3.5 pt-3 sm:px-4 sm:pb-4 sm:pt-4">
  ...
  return (
    <article className="group relative flex h-full min-w-0 flex-col">
      {linkToDetail
        ? <Link href={`/shop/${product.category}/${product.slug}`} className="flex h-full flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-wine focus-visible:ring-offset-4">{content}</Link>
        : <div className="flex h-full flex-col">{content}</div>}
      {linkToDetail ? <ProductSaveButton product={product} className="absolute end-2.5 top-2.5 z-20 size-9 border-white/60 sm:end-3 sm:top-3 sm:size-10" /> : null}
    </article>
  );
}
