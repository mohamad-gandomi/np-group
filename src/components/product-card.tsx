...
export function ProductCard({ product, linkToDetail = true }: { product: Product; linkToDetail?: boolean }) {
  ...
  return (
    <article className="group relative min-w-0">
      {linkToDetail
        ? <Link href={`/shop/${product.category}/${product.slug}`} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-wine focus-visible:ring-offset-4">{content}</Link>
