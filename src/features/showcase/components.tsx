...
import { sectorLabels, type BrandProfile, type Project, type ShowcaseImage } from "./data";
...
export function ShowcaseSchema({ kind, record, records = [] }: { kind: ShowcaseKind; record?: Project | BrandProfile; records?: readonly (Project | BrandProfile)[] }) {
  return <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(showcaseSchema(kind, record, records)).replace(/</g, "\\u003c") }} />;
}
...
export function EditorialImage({ image, hero = false, className = "" }: { image: ShowcaseImage; hero?: boolean; className?: string }) {
  return <figure className={`showcase-figure ${className}`}><div className="showcase-image">{image.src ? <Image src={image.src} alt={image.alt} fill sizes={hero ? "(max-width: 1280px) 100vw, 1280px" : "(max-width: 760px) 100vw, 50vw"} preload={hero} className="object-cover" /> : null}</div><figcaption>{image.caption}</figcaption></figure>;
}
export function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  return <article className="showcase-project-card"><Link href={`/projects/${project.slug}`} aria-labelledby={`project-${project.slug}`}><div className="showcase-card-image">{project.image.src ? <Image src={project.image.src} alt={project.image.alt} fill sizes="(max-width: 760px) 100vw, 50vw" className="object-cover" /> : null}<span className="showcase-image-tag">{sectorLabels[project.sector]}{project.publication.status === "demo" ? " · کانسپت نمایشی" : ""}</span><span className="showcase-card-arrow"><ArrowUpLeft size={24} aria-hidden="true" /></span></div><div className="showcase-project-card-caption"><span className="showcase-index" aria-hidden="true">{faNumber(index + 1).padStart(2, "۰")}</span><div><h3 id={`project-${project.slug}`}>{project.title}</h3><p>{project.description}</p></div></div></Link></article>;
}
export function BrandCard({ brand, products = [] }: { brand: BrandProfile; products?: readonly Product[] }) {
  const labels = [...new Set(products.map((product) => product.categoryTitle).filter((label): label is string => Boolean(label)))];
  return <article className="showcase-brand-card"><Link href={`/brands/${brand.slug}`} aria-labelledby={`brand-${brand.slug}`}><div className={`showcase-wordmark showcase-wordmark--${brand.slug}`}><h3 id={`brand-${brand.slug}`} dir="ltr">{brand.name}</h3><span>{brand.title}</span></div><div className="showcase-brand-card-bottom"><div><p>{labels.join(" / ") || "مجموعه منتخب"}</p><span>{faNumber(products.length)} محصول در کاتالوگ</span></div><ArrowUpLeft size={22} aria-hidden="true" /></div></Link></article>;
}
