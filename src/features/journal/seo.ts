...
export function journalMetadata(post?: JournalPost): Metadata {
  ...
  const imageURL = post ? post.seo.socialImage || post.image : "/placeholders/living.jpg";
  const images = imageURL ? [{ url: imageURL, alt: post?.imageAlt || journalTitle }] : undefined;
  ...
  return {
    ...
    "@graph": [
      ...
      post ? {
        ...
        ...(post.seo.socialImage || post.image ? { image: [absoluteJournalUrl(post.seo.socialImage || post.image!)] } : {}),
