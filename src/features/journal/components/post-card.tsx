  ...
  return (
    <article className="journal-card">
      <Link href={`/blog/${post.slug}`} className="journal-card-link" aria-labelledby={`article-${post.slug}`}>
        ...
        <div className="journal-meta"><span className="journal-category">{categoryLabel(post.category)}</span><span>{journalNumber.format(post.readingTimeMinutes)} دقیقه مطالعه</span></div>
