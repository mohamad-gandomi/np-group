...
export async function CatalogView({ params, categorySlug }: { params: RawSearchParams; categorySlug?: string }) {
  ...
  return (
    <main data-page="catalog">
      ...
      {!category ? <section className="border-b bg-white py-6"><div className="container-shell"><div className="grid auto-cols-[72%] grid-flow-col gap-3 overflow-x-auto pb-2 [scrollbar-width:none] sm:auto-cols-[38%] lg:grid-flow-row lg:grid-cols-4 lg:overflow-visible lg:pb-0">{facets.categories.map((item, index) => <Link key={item.slug} href={`/shop/${item.slug}`} className="group relative min-h-40 overflow-hidden bg-secondary">{item.image ? <Image src={item.image} alt={item.title} fill sizes="(max-width: 640px) 72vw, (max-width: 1024px) 38vw, 25vw" loading={index === 0 ? "eager" : "lazy"} className="object-cover transition-transform duration-500 group-hover:scale-105" /> : null}<div className="image-wash absolute inset-0" /><div className="absolute inset-x-0 bottom-0 p-4 text-white"><p className="text-xs text-white/65">{item.count}</p><h2 className="mt-1 text-lg font-medium">{item.title}</h2></div></Link>)}</div></div></section> : null}
