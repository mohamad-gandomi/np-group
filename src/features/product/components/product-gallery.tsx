...
export function ProductGallery({ images, name, brand }: { images: readonly string[]; name: string; brand: string }) {
  ...
  return <div className="min-w-0">
    <div className="space-y-3" dir="ltr">
      <button type="button" aria-label={hasImages ? "نمایش تصویر بزرگ" : "تصویر محصول هنوز اضافه نشده است"} disabled={!hasImages} className="relative block w-full cursor-zoom-in overflow-hidden bg-secondary p-0 text-start aspect-[4/5] disabled:cursor-default sm:aspect-[7/6] lg:h-[clamp(28rem,calc(100svh-19rem),36rem)] lg:aspect-auto" onClick={() => hasImages && setLightbox(true)} onTouchStart={(event) => setTouchStart(event.touches[0].clientX)} onTouchEnd={onTouchEnd}>
        {image ? <Image src={image} alt={`${name} از برند ${brand}`} fill priority sizes="(max-width: 640px) 100vw, (max-width: 1024px) 78vw, 58vw" className="object-cover" /> : null}
      </button>
