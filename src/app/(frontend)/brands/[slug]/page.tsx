...
import { getShowcaseBrand, getShowcaseBrands, getShowcaseProjects } from "@/features/showcase/payload-showcase-repository";
import { showcaseMetadata } from "@/features/showcase/seo";

type Props = { params: Promise<{ slug: string }> };
export async function generateStaticParams() { return (await getShowcaseBrands()).map(({ slug }) => ({ slug })); }
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const brand = await getShowcaseBrand((await params).slug);
...
export default async function BrandPage({ params }: Props) {
  const brand = await getShowcaseBrand((await params).slug);
  if (!brand) notFound();
  const collection = brand.products;
  const groups = [...new Map(collection.map((product) => [product.category, product.categoryTitle ?? product.category])).entries()]
    .map(([slug, title]) => ({ slug, title }));
  const [projects, brands] = await Promise.all([getShowcaseProjects(), getShowcaseBrands()]);
  const productIDs = new Set(collection.map((product) => product.id));
  const concepts = projects.filter((project) => project.products.some((product) => productIDs.has(product.id))).slice(0, 2);
  const related = brands.filter((item) => item.slug !== brand.slug).slice(0, 3);
  return <main className="showcase" id="showcase-main" tabIndex={-1}><ShowcaseSchema kind="brands" record={brand} records={brands} /><div className="showcase-shell"><Breadcrumbs kind="brands" title={brand.name} />
    <header className={`showcase-brand-hero showcase-brand-hero--${brand.slug}`}><div className="showcase-brand-hero-copy"><p className="showcase-eyebrow">مجموعه {brand.publication.status === "demo" ? "نمایشی" : "منتخب"} / {groups.map((category) => category.title).join(" و ")}</p><h1 dir="rtl"><bdi>{brand.name}</bdi></h1><p className="showcase-brand-tagline">{brand.title}</p><p>{brand.description}</p><a className="showcase-text-link" href="#brand-products">کشف {faNumber(collection.length)} محصول <ArrowDown size={18} aria-hidden="true" /></a></div><figure><div className="showcase-brand-hero-image">{brand.image.src ? <Image src={brand.image.src} alt={brand.image.alt} fill sizes="(max-width: 900px) 100vw, 55vw" preload className="object-cover" /> : null}</div><figcaption>{brand.image.caption}</figcaption></figure></header>
    ...
    <section className="showcase-section" id="brand-products"><div className="showcase-section-heading"><div><p className="showcase-eyebrow">انتخاب از مجموعه</p><h2>محصولات <bdi>{brand.name}</bdi></h2></div><Link href="/shop" className="showcase-text-link">مشاهده کاتالوگ فعلی <ArrowLeft size={18} aria-hidden="true" /></Link></div>{groups.length ? groups.map((category) => <section className="showcase-product-group" id={`collection-${category.slug}`} key={category.slug}><h3>{category.title} <span>{faNumber(collection.filter((product) => product.category === category.slug).length)} محصول</span></h3><ShowcaseSlider label={`محصولات ${brand.name}؛ ${category.title}`}>{collection.filter((product) => product.category === category.slug).map((product) => <ProductCard key={product.id} product={product} />)}</ShowcaseSlider></section>) : <div className="showcase-inline-empty"><p>محصولات این مجموعه هنوز برای نمایش عمومی آماده نشده‌اند.</p><Link href="/shop" className="showcase-text-link">مشاهده همه محصولات <ArrowLeft size={18} aria-hidden="true" /></Link></div>}</section>
    ...
    {!!related.length && <section className="showcase-section"><div className="showcase-section-heading"><h2>نام‌های دیگر، انتخاب‌های تازه</h2><Link href="/brands" className="showcase-text-link">همه برندها <ArrowLeft size={18} aria-hidden="true" /></Link></div><ShowcaseSlider variant="brands" label="برندهای دیگر">{related.map((item) => <BrandCard key={item.slug} brand={item} products={item.products} />)}</ShowcaseSlider></section>}
    <ProjectConsultation />
  </div></main>;
}
