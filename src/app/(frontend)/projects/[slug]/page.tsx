...
export default async function ProjectPage({ params }: Props) {
  ...
  return <main className="showcase" id="showcase-main" tabIndex={-1}>
    ...
    <div className="showcase-shell"><Breadcrumbs kind="projects" title={project.title} />
      ...
      <section id="project-products" className="showcase-section"><div className="showcase-section-heading"><div><p className="showcase-eyebrow">از فضا به محصول</p><h2>برای نزدیک شدن به این حال‌وهوا</h2></div><Link href="/shop" className="showcase-text-link">مشاهده کاتالوگ فعلی <ArrowLeft size={18} aria-hidden="true" /></Link></div><p className="showcase-section-note">این محصولات مرجع نمایشی چیدمان هستند؛ حضور آن‌ها در تصاویر یا کاتالوگ فعلی تأیید نشده است.</p><ShowcaseSlider label="محصولات مرجع نمایشی">{collection.map((product) => <ProductCard key={product.id} product={product} linkToDetail={false} />)}</ShowcaseSlider><div className="showcase-brand-links"><span>مجموعه‌های مرتبط</span>{brandNames.map((name) => <Link key={name} href={brandPath(name)} dir="ltr">{name}<ArrowUpLeftSmall /></Link>)}</div></section>
