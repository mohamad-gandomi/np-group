...
import { getShowcaseProject, getShowcaseProjects } from "@/features/showcase/payload-showcase-repository";
...
export async function generateStaticParams() { return (await getShowcaseProjects()).map(({ slug }) => ({ slug })); }
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const project = await getShowcaseProject((await params).slug);
...
export default async function ProjectPage({ params }: Props) {
  const project = await getShowcaseProject((await params).slug);
  if (!project) notFound();
  const collection = project.products;
  const brandNames = [...new Set(collection.map((product) => product.brand))];
  const [article, projects] = await Promise.all([
    project.articleSlug ? getJournalPost(project.articleSlug) : Promise.resolve(null),
    getShowcaseProjects(),
  ]);
  ...
  return <main className="showcase" id="showcase-main" tabIndex={-1}>
    <ShowcaseSchema kind="projects" record={project} records={projects} />
    <div className="showcase-shell"><Breadcrumbs kind="projects" title={project.title} />
      ...
      <section className="showcase-section" id="project-gallery"><div className="showcase-section-heading"><div><p className="showcase-eyebrow">دفتر تصویر</p><h2>برداشت‌های دیگر از یک ایده</h2></div><p>تصاویر مستقل برای مطالعه رنگ، فرم و چیدمان</p></div><ShowcaseSlider variant="gallery" label="تصاویر الهام">{project.gallery.map((item, index) => <EditorialImage key={`${item.src}-${index}`} image={item} />)}</ShowcaseSlider></section>
      {!!collection.length && <section id="project-products" className="showcase-section"><div className="showcase-section-heading"><div><p className="showcase-eyebrow">از فضا به محصول</p><h2>برای نزدیک شدن به این حال‌وهوا</h2></div><Link href="/shop" className="showcase-text-link">مشاهده کاتالوگ فعلی <ArrowLeft size={18} aria-hidden="true" /></Link></div><p className="showcase-section-note">محصولات پیشنهادی این فضا را می‌توانید در کاتالوگ بررسی کنید.</p><ShowcaseSlider label="محصولات پیشنهادی">{collection.map((product) => <ProductCard key={product.id} product={product} />)}</ShowcaseSlider><div className="showcase-brand-links"><span>مجموعه‌های مرتبط</span>{brandNames.map((name) => { const product = collection.find((item) => item.brand === name); return <Link key={name} href={product?.brandSlug ? `/brands/${product.brandSlug}` : `/shop?brand=${encodeURIComponent(name)}`} dir="ltr">{name}<ArrowUpLeftSmall /></Link>; })}</div></section>}
